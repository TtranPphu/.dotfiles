...
- **Commit format:** `[Component] - Summary`. Component in title-case square brackets, dash separator, sentence-style summary. Examples: `[Hypr]`, `[Nvim]`, `[Tmux]`, `[Zsh]`, `[Starship]`. Details after blank line if needed. End the body with a blank line followed by `Claude - <model>` where `<model>` matches the system prompt (e.g., `deepseek-v4-flash[1m]`). Do NOT use `Co-Authored-By` or other formats.
- **No auto-committing:** Only commit when explicitly asked. Stage changes when asked, but never create a commit without a direct instruction.
