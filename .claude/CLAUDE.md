...
- **Commit format:** `[Component] - Summary`. Component in title-case square brackets, dash separator, sentence-style summary. Examples: `[Hypr]`, `[Nvim]`, `[Tmux]`, `[Zsh]`, `[Starship]`. Details after blank line if needed. End the body with a blank line followed by `Claude - <model>` to identify Claude Code as the agent.
