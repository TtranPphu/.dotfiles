-- Visual whitespace display in visual mode (patched version)
-- Upstream visual-whitespace.nvim is missing eol support in read_opt_listchars()
-- This includes the full patched module and setup
...
-- FIX: read_opt_listchars() now includes eol (upstream omits it)
local function read_opt_listchars()
  local lcs = v.opt.listchars:get()
  if lcs.tab and #lcs.tab > 1 then
    lcs.tab = v.fn.strcharpart(lcs.tab, 0, 1)
  end
  return {
    space = lcs.space,
    tab = lcs.tab,
...
local function marks_for_line(bufnr, row, s_col, e_col, nl_char)
  ...
  local match_eol = match_types.eol
  ...
  local eol_glyph = list_chars.eol
  ...
  -- FIX: Use eol_glyph if match_eol is enabled, otherwise use fileformat char
  if e_col > line_len then
    local final_char = (match_eol and eol_glyph) or nl_char
    marks[#marks + 1] = { row, line_len + 1, final_char }
  end
  return marks
end

local function provider_on_start()
  if not STATE.user_enabled or not is_visual_mode() or not is_allowed_ft_bt() then
    return false
  end
  local mode = fn.mode(1)
  region = fn.getregionpos(fn.getpos('v'), fn.getpos('.'), { type = mode, eol = true })
  selection = {}
...
function M.setup(user_cfg)
  ...
  CFG = v.tbl_deep_extend('force', { list_chars = lcs_defaults }, BASE_CFG, user_cfg)
  CFG.ignore = {
