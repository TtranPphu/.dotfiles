...
- **Agent signature:** End the body with a blank line followed by an agent signature identifying the agent that generated the commit. Copilot CLI uses `Copilot - <model>`, Claude Code uses `Claude - <model>`. The `<model>` must match the full string from the system prompt exactly (e.g., `deepseek-v4-flash[1m]`).
