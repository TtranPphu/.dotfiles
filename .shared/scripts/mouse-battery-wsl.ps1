# Read mouse battery level via Windows Bluetooth LE APIs
# Output: "<percent>" or exits 1 on failure
#
# Matches the Logitech Lift by PnP friendly name, then reads the standard
# BLE Battery Service (0x180F / 0x2A19) through WinRT, the same way
# keyboard-battery-wsl.ps1 does for the ZMK keyboard.
...
$btAddr = $null
try {
  $device = Get-PnpDevice -Class Bluetooth | Where-Object { $_.FriendlyName -like "*LIFT*" -and $_.Status -eq "OK" } | Select-Object -First 1
